...
- **Status (2026-07-15): BUILT, ships in placeholder state — go-live blocked only on founder-supplied content + provisioning.** Implemented per the E11.1.4 decision: `DataRoomGate` (name/email/firm required, check size optional — copy.md §7 revised with the decision) → lead stored in `data_room_requests` (new migration, anon insert-only RLS, no unique on email so return visits re-mint links) → instant in-place unlock listing the catalog slots from `lib/api/data-room.ts` with 15-minute signed URLs minted via a new server-only `createSupabaseAdminClient` (service-role, storage signing ONLY). Slots pending files render "In preparation"; a missing service key degrades to pending, never breaks. `noindex` + robots-disallowed (already) + absent from sitemap/nav. Analytics: `data_room_request` + `data_room_download` (PRD §10 "Investor PDF Downloads") added to the typed union. Gate inline in the marketing shell — the E3.2.3 `Dialog` dependency turned out unnecessary. **Go-live checklist (operator steps + the two required PDFs, pitch deck + one-pager): [`docs/deployment/data-room-storage.md`](../deployment/data-room-storage.md); flipping each slot's `storagePath` from `null` is the only code change.**
...
- **Status (2026-07-15): DECIDED (user-approved from a compared recommendation): email-gated.** Name/firm/email form (all required) → instant access; the API route stores the lead via the same insert-only Supabase seam as the waitlist (its own `data_room_requests` table) and returns **time-limited Supabase Storage signed URLs** — the PDFs are never public paths. Two governing disciplines: (1) **content discipline is the real security** — only competitor-safe collateral (deck, one-pager, the already-published unit economics) goes in; diligence-grade material (models, per-category TAM tables, cap table) stays in founder-controlled DocSend, shared 1:1; (2) fire the PRD §10 "Investor PDF Downloads" GA4 event on download (extend the `lib/analytics` union). Rejected for this stage: verified access (needs email-sending infra + auth the frontend-only architecture doesn't have, filters little) and manual approval (breaks the IA §5 one-sitting cold-investor journey; DocSend already does this better for real diligence). Upgrade path: the same form can later swap to magic-link verification without changing the page. Also recorded in PRD §22.6. **Unblocks E11.1.3 — but note E11.1.3 has a founder-supplied content dependency: the actual deck/one-pager PDFs are business artifacts that must come from the founders (never invented); the page/gate/seam can be built with a placeholder slot awaiting them.**
...
- **Superseded (2026-07-08):** the E14.1 switch to `@next/third-parties/google` forecloses this — the integration hard-codes `afterInteractive` and doesn't expose `strategy="worker"`. Accepted trade-off (see `frontend.md` §8). Reopen only if GA4 main-thread cost shows up as a real Lighthouse regression, in which case reconsider a hand-rolled loader.
