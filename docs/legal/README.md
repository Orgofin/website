...
**The 24-month retention window is enforced as of 2026-07-25.** The nightly pg_cron purge in [`20260724120000_lead_retention_expiry.sql`](../../supabase/migrations/20260724120000_lead_retention_expiry.sql) is applied and scheduled on **both** Supabase projects — `cron.job` reports `purge-expired-leads`, `15 3 * * *`, `active = true` on each. `/privacy` §8 now describes something that happens, closing the last say/do gap on these pages. Runbook: [`../deployment/data-retention.md`](../deployment/data-retention.md).
...
- [x] ~~**Founder/infra:** **apply** the retention migration to both Supabase projects~~ — done 2026-07-25, schedule confirmed active on prod and non-prod.
