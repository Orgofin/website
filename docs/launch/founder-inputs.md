...
| --- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| A1  | **Registered legal entity name**    | `/privacy` and `/terms` currently name the _trading_ name. If the registered entity differs, both published policies name the wrong party — the one defect on a legal page that undermines every other clause. | `LEGAL_ENTITY_NAME` in [`lib/legal/constants.ts`](../../src/lib/legal/constants.ts) (marked `TODO(legal)`) |
| A2  | **Registered office address**       | Both policies currently render an honest "not yet published, write to us instead" line. DPDP expects a contactable address. Deliberately `null` rather than a plausible-looking placeholder.                   | `LEGAL_REGISTERED_ADDRESS` in [`lib/legal/constants.ts`](../../src/lib/legal/constants.ts)                 |
