...
| **Contact** `/contact`                        | Hero → six routed enquiry channels (product, investors, partnerships, careers, press, privacy) → the address itself + response promise → waitlist band. **No form and no demo request** — see §7                                                 |
...
- Sitemap priority: Home 1.0 → Platform/Products/Company Brain/Vision 0.8 → Investors/Blog 0.7 → Team/About/Security 0.6 → Careers/Contact 0.5 → legal 0.3.
- **Canonical:** `/platform` is an overview hub, not a second home for the deep-dive narratives — it must never reuse `/company-brain`'s or Home Ch.6's headline copy verbatim. Its Company Brain section summarises; its agents section shows the named-agent roster (additive) rather than repeating Home's CEO vignette. **`/security` is canonical for the compliance narrative** and **`/products` is canonical for the eight-suite grid** — `/platform`'s `SecurityLayer` and `SuitesLayer` are teasers that render a one-line summary and deep-link out (the same teaser→canonical pattern as Home's `VisionTeaser` → `/vision`), never the pillars or the suite grid verbatim. On `/products`, only HRMS gets a per-module breakdown (the one suite with a documented module list in `.claude/knowledge/hrms.md`); the other seven stay at suite level, and availability is asserted only at the suite level pending founder confirmation of module-level MVP status (`hrms.md` open question).
...
**`/contact` routes rather than collects (2026-07-25).** The copy deck §13 specified a five-field form ending in "Request a Demo". Neither shipped, and both departures are deliberate:

- **No form.** It would have been a third PII collection point — a table, a migration, an API route, rate limiting, plus `/privacy` and inventory updates — and it would have sat broken in production until someone applied that migration. Six cards with pre-filled `mailto:` subjects do the routing a form's "enquiry type" dropdown would have done, work on deploy, and add nothing to what we store. Revisit when inbound volume makes a plain inbox unmanageable; the copy deck's field list is still there when it does.
- **No demo request.** Nothing is publicly available (founder statement 2026-07-22 — the fact that removed "available now" from `/products`). The page offers a conversation, which is deliverable. This applies to `/products`' CTA too.
- **The `/partners` card is the only one with no page to link.** It takes the email route rather than linking a 404 — the never-link-a-404 rule (§4) is what lets an unbuilt page still receive enquiries.

## Current Status

Implemented routes: `/` (the full Home narrative spine), `/platform` (Platform overview hub), `/products` (canonical eight-suite grid + HRMS lifecycle), `/security` (canonical Security & Compliance narrative), `/vision` (canonical vision narrative, E11.1.1), `/investors` (full investor thesis, E11.1.2), `/about` (origin story), `/careers`, `/privacy` and `/terms` (2026-07-24), `/contact` (2026-07-25), and `/waitlist/thank-you` (noindex confirmation). **The header nav no longer contains any `#` placeholders** — every visible nav and footer link resolves to a real page (2026-07-20). Everything else in the §1 tree is planned, not built; new nav entries appear only when their page ships.
...
- [x] ~~**Wire `/security`'s remaining deferred §4 CTA:** "Talk to our team about your compliance requirements" → `/contact`~~ — **done 2026-07-25** when `/contact` shipped. It renders as a line between `ComplianceDepth` and the waitlist band, alongside the "Read our Privacy Policy" CTA that shipped 2026-07-24 — deliberately not inside `CTABand`, which is shared by every non-legal page.
- [x] ~~**Wire `/products`'s deferred "Request a Demo" CTA (§3) to `/contact`**~~ — **done 2026-07-25, but not as a demo request.** Nothing is publicly available (founder statement 2026-07-22), so the link offers a conversation rather than a demo of software that doesn't exist; wording it the deck's way would have undone the accuracy correction `/products` exists to carry.
