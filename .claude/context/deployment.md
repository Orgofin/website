...
**⚠️ Dependabot _security_ PRs open against `main`, not `dev` — always check the base branch before merging one (2026-08-06).** `target-branch: dev` in [`dependabot.yml`](../../.github/dependabot.yml) governs **version** updates only. Security updates ignore it entirely and always target the **default branch**, which here is `main`. Merging one where it lands puts a dependency change straight into production that `dev` and `uat` never saw — inverting the promotion flow and creating drift the next cycle's `uat`/`dev` comparison cannot detect.

Tell them apart by the branch name: version updates carry a `/dev/` segment (`dependabot/npm_and_yarn/dev/npm-minor-patch-…` → base `dev` ✅), security updates do not (`dependabot/npm_and_yarn/undici-7.29.0` → base `main` ⚠️). **Correct handling: close the PR and re-apply the same bump on a branch off `dev`**, as an `override` if it is transitive (this is how #184/#181 were resolved). Note also that these PRs are cut against `main`'s lockfile, so after a promotion several of them go stale and become redundant on arrival — #187 and #188 were closed unmerged for exactly this reason, `main` having already received the equivalent floors via #186.
...
**The `undici` and `ip-address` overrides, and why one of them is version-scoped (2026-08-06).** Two high-severity dev-tree advisories arrived together: `undici` (five findings — response desynchronization, cache-directive information disclosure, CRLF injection, cookie attribute injection) covering `7.0.0 – 7.28.0`, and `ip-address` (three SSRF / trust-boundary bypasses) covering `<=10.3.0`. Neither is in the production tree — `undici` arrives via `shadcn` and `jsdom`, `ip-address` via `@lhci/cli` and `shadcn`, all three `devDependencies` — so neither blocked the gate.

`ip-address` has a single installed instance and both consumers (`express-rate-limit@^10.2.0`, `socks@^10.1.1`) accept the fix, so a plain `^10.4.0` floor is safe. **`undici` is the interesting one: it is pinned as `"undici@7"`, not `"undici"`.** The tree holds two independent copies — `shadcn`/`@dotenvx/dotenvx` on 7.x, and **`jsdom@30` on `^8.9.0`** — and the advisory only covers the 7.x line. A blanket `"undici": "^7.29.0"` would have forced jsdom's `8.9.0` **down a major**, breaking the entire Vitest suite in the name of fixing a vulnerability jsdom never had. The `@version` selector patches the affected line and leaves the unaffected one alone. **Before adding any override, check whether the package is installed more than once and whether every consumer's declared range accepts the floor** — this is the same failure mode the `@hono/node-server` note describes, reached from the opposite direction.

**The `next@16.3.0` upgrade, and the production tree reaching zero (2026-08-06).** `postcss` GHSA-fxqj-rqcc-2cmp (moderate — attacker-controlled `sourceMappingURL` reads arbitrary `.map` files when `from` is unset) was the production tree's last remaining finding, and it was **not fixable by an override**: the advisory covers `<=8.5.22`, while `next@16.2.12` pins `postcss@8.4.31` exactly. `next@16.3.0` pins `postcss@8.5.23`, so upgrading both clears the advisory **and** satisfies the documented removal condition for the `overrides.next.postcss` entry, which is now deleted.

Done as a deliberate upgrade rather than via `npm audit fix --force`, which would have taken the same bump as an unreviewed side effect. 16.2 → 16.3 is a **minor** release with no breaking-changes section; its deprecations (middleware, edge runtime, `experimental.useCache`/`dynamicIO`, TS `baseUrl`/`moduleResolution: "node"`/`downlevelIteration`) were each checked against this repo and **none apply** — there is no middleware, no edge runtime, no `experimental.*` flag in [`next.config.ts`](../../next.config.ts), and `tsconfig.json` already uses `moduleResolution: "bundler"`. `eslint-config-next` and `@next/third-parties` moved in lockstep, as they must.

**The production tree now reports `found 0 vulnerabilities`** — the first time it has been completely clean, not merely clean at the blocking threshold. The full tree carries `tmp` (high) and `uuid` (moderate), **both reached only through `@lhci/cli`** — the Lighthouse runner added 2026-08-02, which executes in CI and ships nothing. Those are precisely the case the `continue-on-error` full-tree step exists to surface without blocking; do not "fix" them by relaxing the blocking gate.
...
- [x] Remove the `package.json` `overrides` entry forcing `next`'s nested `postcss` to `^8.5.10` (added 2026-07-15 for the Dependabot XSS alert, GHSA postcss < 8.5.10) once a Next release stops pinning `postcss@8.4.31` — **done 2026-08-06**: `next@16.3.0` pins `postcss@8.5.23` directly, so the override became redundant and was deleted.
...
- [ ] Drop the `undici@7` and `ip-address` overrides (added 2026-08-06, see above) once `shadcn`, `@dotenvx/dotenvx`, and `@lhci/cli` resolve patched versions unaided. **Keep the `@7` selector if the override is ever widened** — `jsdom` holds an independent `undici@8.x` that the advisory does not cover.
...
**Last Updated:** 2026-08-06 (`next@16.3.0` — production tree reaches zero findings, `postcss` override retired; `undici`/`ip-address` overrides + version-scoped selector rule; Dependabot security-PR base-branch warning)
